...
function cleanDataset(filePath) {
  ...
  initialNats.forEach(q => {
    ...
    // Attempt answer extraction if correct is defaulted to 0, -1, or missing
    if (q.correct === 0 || q.correct === -1 || q.correct === null || q.correct === undefined || q.correct === '') {
...
function main() {
  ...
  // Sync enriched answers to cleaned file if available
  if (fs.existsSync(FILE_PRIMARY) && fs.existsSync(FILE_CLEANED)) {
    const primaryData = JSON.parse(fs.readFileSync(FILE_PRIMARY, 'utf8'));
    const cleanedData = JSON.parse(fs.readFileSync(FILE_CLEANED, 'utf8'));
    const primaryMap = new Map(primaryData.map(q => [q.id, q.correct]));

    let synced = 0;
    cleanedData.forEach(q => {
      if (primaryMap.has(q.id)) {
        const val = primaryMap.get(q.id);
        if (typeof val === 'number' && val !== -1 && val !== 0 && (q.correct === 0 || q.correct === -1)) {
          q.correct = val;
          synced++;
        }
      }
    ...
    if (synced > 0) {
      fs.writeFileSync(FILE_CLEANED, JSON.stringify(cleanedData, null, 2), 'utf8');
      console.log(`Synced ${synced} enriched correct values to cleaned_mechEngQuestions.json`);
    }
  }
